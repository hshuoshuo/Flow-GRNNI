...
class FlowDis(PandasDataset):
    ...
    def load_raw(self, small=False):
        ...
        direction = pd.DataFrame(pd.read_hdf(path, 'direction'))
    ...
    def load(self, impute_nans=True, small=False, masked_sensors=None):
        ...
        flow_dir_matrix = direction.values
        flow_dir_matrix[flow_dir_matrix == 0] = np.inf
        np.fill_diagonal(flow_dir_matrix, 0)
        n = flow_dir_matrix.shape[0]
        # for k in range(n):
        #     for i in range(n):
        #         for j in range(n):
        #             flow_dir_matrix[i][j] = min(flow_dir_matrix[i][j], flow_dir_matrix[i][k] + flow_dir_matrix[k][j])
        while True:
            previous_matrix = flow_dir_matrix.copy()
            for k in range(n):
                for i in range(n):
                    for j in range(n):
                        flow_dir_matrix[i][j] = min(flow_dir_matrix[i][j], flow_dir_matrix[i][k] + flow_dir_matrix[k][j])
            if np.array_equal(flow_dir_matrix, previous_matrix):
                break
        
        # Compute shortest path distances using Floyd-Warshall algorithm
        # dist = scipy.sparse.csgraph.floyd_warshall(flow_dir_matrix, directed=True)
        dist=flow_dir_matrix
        # print(dist)
        return df, dist, mask
    ...
    def get_similarity(self, thr=0.1, include_self=False, force_symmetric=False, sparse=False, **kwargs):
        dist_no_inf = np.where(np.isinf(self.dist), np.nan, self.dist)
        theta = np.nanstd(dist_no_inf)
        # print("theta is:", theta)
        adj = thresholded_gaussian_kernel(dist_no_inf, theta=theta, threshold=thr)
