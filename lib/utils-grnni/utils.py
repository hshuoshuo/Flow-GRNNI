...
# max_seq=1, min_seq=1 -> point missing
# max_seq=48, min_seq=2 -> block missing
...
def compute_mean(x, index=None):
    ...
    cond1 = [df_mean.index.year, df_mean.index.month]

    conditions = [cond0, cond1, cond0[1:], cond1[1:]]
...
def thresholded_gaussian_kernel(distances, theta, threshold):
        kernel = np.exp(-distances**2 / (2 * theta**2))
